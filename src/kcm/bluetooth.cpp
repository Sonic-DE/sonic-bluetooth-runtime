...
K_PLUGIN_FACTORY_WITH_JSON(KCMBluetoothFactory, "kcm_bluetooth.json",
                           registerPlugin<Bluetooth>();
                           registerPlugin<BluetoothModuleData>();)
...
Bluetooth::Bluetooth(QObject *parent, const KPluginMetaData &data)
  ...
  qmlRegisterSingletonInstance("org.kde.bluedevil.kcm", 1, 0,
                               "FileReceiverSettings",
                               FileReceiverSettings::self());
  qmlRegisterSingletonInstance("org.kde.bluedevil.kcm", 1, 0, "GlobalSettings",
                               GlobalSettings::self());
}
...
void Bluetooth::runWizard(QQuickItem *context) {
  auto runIt = [this](const QString &windowHandle) {
    auto *job = new KIO::CommandLauncherJob(
        QStringLiteral("bluedevil-wizard"),
        {QStringLiteral("--parentWindow"), windowHandle});
  ...
  QWindow *actualWindow =
      QQuickRenderControl::renderWindowFor(context->window());

  // In full Qt Quick apps like plasma-settings, you just use the window
  // directly
  if (!actualWindow) {
...
void Bluetooth::runSendFile(const QString &ubi) {
  auto *job = new KIO::CommandLauncherJob(QStringLiteral("bluedevil-sendfile"),
                                          {QStringLiteral("-u"), ubi});
...
void Bluetooth::checkNetworkConnection(const QStringList &uuids,
...
void Bluetooth::checkNetworkInternal(const QString &service,
                                     const QString &address) {
...
void Bluetooth::setupNetworkConnection(const QString &service,
                                       const QString &address,
  ...
  msg << address;
  msg << service;
  msg << i18nc("DeviceName Network (Service)", "%1 Network (%2)", deviceName,
               service);
